...
async function prerender() {
  const template = fs.readFileSync(path.resolve(__dirname, 'dist/static/index.html'), 'utf-8');
  ...
  for (const url of routesToPrerender) {
    ...
    const filePath = path.resolve(__dirname, `dist/static${url === '/' ? '/index' : url}/index.html`);

...
var generateQuestionPaperWord = async (questions, settings) => {
	const children = [];
	const template = settings.template || "classic";
	const isClassic = template === "classic";
	const isCompact = template === "compact";
	if (settings.institutionName) children.push(new Paragraph({
		text: settings.institutionName.toUpperCase(),
		heading: HeadingLevel.HEADING_1,
		alignment: isClassic ? AlignmentType.CENTER : AlignmentType.LEFT,
		spacing: { after: 100 }
	}));
	if (settings.examTitle) children.push(new Paragraph({
	...
	if (isCompact) {
		if (settings.subject) children.push(new Paragraph({ children: [new TextRun({
		...
		children.push(new Paragraph({
			text: "Reg No: _____________________",
			spacing: { after: 100 }
		}));
	} else {
		...
		const rows = [];
		if (settings.subject || settings.date) rows.push(createRow(settings.subject ? `Subject: ${settings.subject}` : "", settings.date ? `Date: ${settings.date}` : "", true));
		if (settings.duration || settings.totalMarks) rows.push(createRow(settings.duration ? `Duration: ${settings.duration}` : "", settings.totalMarks ? `Total Marks: ${settings.totalMarks}` : ""));
		...
		if (rows.length > 0) children.push(new Table({
			rows,
			width: {
				size: 100,
				type: WidthType.PERCENTAGE
	...
	if (settings.instructions) children.push(new Paragraph({
		children: [new TextRun({
			text: "Instructions:\n",
			bold: true
	...
	questions.forEach((q, index) => {
		...
		else if (numCols === 2) children.push(new Table({
			rows: [new TableRow({ children: [new TableCell({
				children: [new Paragraph({ text: `A. ${q.options["A"]}` })],
				width: {
					size: 50,
					type: WidthType.PERCENTAGE
				},
				...
				children: [new Paragraph({ text: `B. ${q.options["B"]}` })],
				width: {
					size: 50,
					type: WidthType.PERCENTAGE
				},
				...
				children: [new Paragraph({ text: `C. ${q.options["C"]}` })],
				width: {
					size: 50,
					type: WidthType.PERCENTAGE
				},
				...
				children: [new Paragraph({ text: `D. ${q.options["D"]}` })],
				width: {
					size: 50,
					type: WidthType.PERCENTAGE
				},
		...
		else if (numCols === 4) children.push(new Table({
	...
	saveAs(await Packer.toBlob(doc), settings.subject ? `${settings.subject.replace(/[^a-z0-9]/gi, "_")}_Question_Paper.docx` : "Question_Paper.docx");
};
var generateAnswerKeyWord = async (questions, settings) => {
	const children = [];
	if (settings.institutionName) children.push(new Paragraph({
		text: settings.institutionName.toUpperCase(),
		heading: HeadingLevel.HEADING_1,
	...
	if (settings.subject) children.push(new Paragraph({
		text: `Subject: ${settings.subject}`,
		spacing: { after: 100 }
	}));
	if (settings.examTitle) children.push(new Paragraph({
	...
	saveAs(await Packer.toBlob(doc), settings.subject ? `${settings.subject.replace(/[^a-z0-9]/gi, "_")}_Answer_Key.docx` : "Answer_Key.docx");
};
//#endregion
		...
		children: /* @__PURE__ */ jsxs("div", {
			...
			children: [
				...
				/* @__PURE__ */ jsxs("button", {
					...
					children: [/* @__PURE__ */ jsx(FileText, { className: "w-5 h-5" }), "Question Paper (Word)"]
				}),
				/* @__PURE__ */ jsxs("button", {
				...
				/* @__PURE__ */ jsxs("button", {
					...
					children: [/* @__PURE__ */ jsx(CheckSquare, { className: "w-5 h-5" }), "Answer Key (Word)"]
				})
			]
		})
	});
